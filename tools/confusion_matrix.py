...
# Thêm đường dẫn project vào sys.path để import được torchlight và net
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from torchlight import import_class

# ==========================================
# 1. CUSTOM FEEDER (Bạn điền data vào đây)
# ==========================================
class CustomFeeder(Dataset):
    def __init__(self, rgb_path, transform=None):
        ...
        # Transform mặc định cho ResNet (224x224)
    ...
    def __getitem__(self, index):
        ...
        label = int(info['label']) - 1 # Giả sử label trong data từ 1->N, model cần 0->N-1
        
        # Đường dẫn ảnh
        # Lưu ý: Đảm bảo đường dẫn rgb_path có dấu / ở cuối khi truyền arg
...
def load_model(config_path, weights_path, device):
    ...
    try:
        ...
        print("Strict loading failed, trying partial load...")
...
def evaluate_model(model, data_loader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc='Evaluating'):
            # Batch từ CustomFeeder trả về: (rgb, label, filename)
            data, label, _ = batch
...
def plot_confusion_matrix(y_true, y_pred, num_classes, output_path):
    # Tạo tên class (Class 1, Class 2...) hoặc tên cụ thể nếu có
    class_names = [f'Class {i+1}' for i in range(num_classes)]
    
    cm = confusion_matrix(y_true, y_pred, normalize='true') # Normalize để xem phần trăm
...
def main():
    ...
    parser.add_argument('--config', required=True, help='Path to config file (để biết kiến trúc model)')
    parser.add_argument('--weights', required=True, help='Path to .pt model weights')
    parser.add_argument('--rgb_path', required=True, help='Folder chứa ảnh png')
    ...
    # 2. Load Data (Sử dụng CustomFeeder ở trên)
    print(f"Initializing CustomFeeder with images from: {args.rgb_path}")
    dataset = CustomFeeder(rgb_path=args.rgb_path)
    
    if len(dataset) == 0:
        print("CẢNH BÁO: data_dict đang trống! Hãy mở file code và điền dữ liệu vào self.data_dict.")
        return
    ...
    # Lấy số class từ config hoặc từ dữ liệu
    num_class = config.get('model_args', {}).get('num_class', len(np.unique(y_true)))
    
    print("\nClassification Report:")
    print(classification_report(y_true, y_pred, digits=4))
    
    # 5. Plot
    plot_confusion_matrix(y_true, y_pred, num_class, args.output)
